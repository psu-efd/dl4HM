...
import sys
#sys.path.append("../../bathy_inversion_real_case")  #for execution in terminal
#print(sys.path)
...
def predict(config_filename, trained_model_data_filename, nPlotSamples=1):
    ...
    #Get test data
    test_dataset = data_loader.get_test_data(bHasInDomainMask=True)
    ...
    for record in test_dataset:
        ID, zb_test, uvWSE_test, bInDomain = record
        ...
        if ID.numpy() in choices:
            ...
            # make plot
            specific_utilities.plot_one_prediction(ID, b_uv_only, uvWSE_pred, uvWSE_test, zb_test, bInDomain,
                                                   var_min_max = data_loader.get_variables_min_max(), bPlot_zb=True, bPlot_dimless=True)
...
def invert_L_curve(config_filename, trained_model_data_filename):
    ...
    nAlpha = 7   #number of alpha_slope values
    alpha_slope = np.logspace(-1.7, 0.4, num=nAlpha)  #linearly distributed in log space
    print("alpha_slope = ", alpha_slope)

    #inital bed
    #zb_init = (np.random.random(inverter.get_input_data_shape()) - 0.5) * 0.01
    # load the initial zb for multiple inversions
    sampled_elevations_for_inversion_init = np.load("sampled_elevations_for_inversion_init.npz")
    sampled_elevations_for_inversion_init = sampled_elevations_for_inversion_init['elevations']
...
def prior_info():
    ...
    inversion_data = np.load("inversion_case_uvWSE_1000.npz")
    bInDomain_data = np.load("bInDomain.npz")

    zb_truth = inversion_data['zb'].squeeze()

    bIndomain = bInDomain_data['bInDomain']
    ...
    print("before masking out boundary:")
    print("min, max of slope_x =", dzb_dx.min(), dzb_dx.max())
    print("min, max of slope_y =", dzb_dy.min(), dzb_dy.max())

    dmasks_dx = 1 - np.abs(np.diff(bIndomain, axis=1))
    dmasks_dy = 1 - np.abs(np.diff(bIndomain, axis=0))

    dzb_dx = np.multiply(dzb_dx, dmasks_dx)
    dzb_dy = np.multiply(dzb_dy, dmasks_dy)

    print("after masking out boundary:")
    print("min, max of slope_x =", dzb_dx.min(), dzb_dx.max())
    print("min, max of slope_y =", dzb_dy.min(), dzb_dy.max())
...
if __name__ == '__main__':
    ...
    trained_model_data_filename = "./experiments/2023-04-17/uvWSE/checkpoints/uvWSE.hdf5"

    #specific_utilities.calculate_relative_and_absolute_errors_for_all_test_cases()

    predict(config_filename=config_filename, trained_model_data_filename=trained_model_data_filename, nPlotSamples=4)
