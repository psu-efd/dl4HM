...
def plot_one_prediction(ID, b_uv_only, vel_WSE_pred, vel_WSE_test, zb_test, bInDomain, var_min_max, bPlot_zb=True, bPlot_dimless=False):
    """
    ...
    if bPlot_zb:
        fig, axs = plt.subplots(4, 3, figsize=(3 * 8, 4 * 2), sharex=True, sharey=True, facecolor='w', edgecolor='k')
    else:
        fig, axs = plt.subplots(3, 3, figsize=(3 * 6, 3 * 2), sharex=True, sharey=True, facecolor='w', edgecolor='k')
    ...
    if bPlot_dimless:
        ...
        cf_vel_x_sim = axs[0, 0].contourf(X, Y, ma.masked_array(scale_back(np.squeeze(vel_WSE_test[:, :, 0]), vel_x_min, vel_x_max),1-bInDomain), local_levels,
    ...
    if bPlot_dimless:
        ...
        cf_vel_x_pred = axs[0, 1].contourf(X, Y, ma.masked_array(scale_back(np.squeeze(vel_WSE_pred[:, :, :, 0]), vel_x_min, vel_x_max), 1-bInDomain), local_levels,
                                           vmin=local_levels.min(), vmax=local_levels.max(), cmap=plt.cm.jet, extend='both')
    ...
    #calculate RMSE
    rmse_vel_x = np.sqrt((vel_x_diff ** 2).mean() * (bInDomain.shape[0]*bInDomain.shape[1]) / np.sum(bInDomain) )

    cf_vel_x_diff = axs[0, 2].contourf(X, Y, ma.masked_array(vel_x_diff,1-bInDomain), diff_levels, vmin=v_min, vmax=v_max, cmap=plt.cm.jet, extend='both')  #cm: PRGn
    ...
    if bPlot_dimless:
        ...
        cf_vel_y_sim = axs[1, 0].contourf(X, Y, ma.masked_array(scale_back(np.squeeze(vel_WSE_test[:, :, 1]), vel_y_min, vel_y_max),1-bInDomain),
                                          local_levels,
                                          vmin=local_levels.min(), vmax=local_levels.max(), cmap=plt.cm.jet, extend='both')
    ...
    if bPlot_dimless:
        ...
        cf_vel_y_pred = axs[1, 1].contourf(X, Y, ma.masked_array(scale_back(np.squeeze(vel_WSE_pred[:, :, :, 1]), vel_y_min, vel_y_max),1-bInDomain),
                                          local_levels,
                                          vmin=local_levels.min(), vmax=local_levels.max(), cmap=plt.cm.jet, extend='both')
    ...
    # calculate RMSE
    rmse_vel_y = np.sqrt((vel_y_diff ** 2).mean() * (bInDomain.shape[0]*bInDomain.shape[1]) / np.sum(bInDomain))

    cf_vel_y_diff = axs[1, 2].contourf(X,Y, ma.masked_array(vel_y_diff,1-bInDomain), diff_levels, vmin=v_min, vmax=v_max, cmap=plt.cm.jet, extend='both')  # cm: PRGn
    ...
    if b_uv_only:
        ...
        local_levels = np.linspace(WSE_min - 0.01, WSE_max + 0.01, 51)

        if bPlot_dimless:
            cf_WSE_sim = axs[2, 0].contourf(X, Y, np.squeeze(vel_WSE_test[:, :, 2]), levels, vmin=min, vmax=max,
                                              cmap=plt.cm.jet, extend='both')
        else:
            cf_WSE_sim = axs[2, 0].contourf(X, Y, ma.masked_array(scale_back(np.squeeze(vel_WSE_test[:, :, 2]), WSE_min, WSE_max),1-bInDomain),
                                              local_levels,
                                              vmin=local_levels.min(), vmax=local_levels.max(), cmap=plt.cm.jet, extend='both')
    ...
    if b_uv_only:
        ...
        if bPlot_dimless:
            ...
            cf_WSE_pred = axs[2, 1].contourf(X, Y, ma.masked_array(scale_back(np.squeeze(vel_WSE_pred[:, :, :, 2]), WSE_min, WSE_max),1-bInDomain),
                                              local_levels,
                                              vmin=local_levels.min(), vmax=local_levels.max(), cmap=plt.cm.jet, extend='both')
    ...
    # calculate RMSE
    rmse_WSE = np.sqrt((WSE_diff ** 2).mean() * (bInDomain.shape[0]*bInDomain.shape[1]) / np.sum(bInDomain))

    if b_uv_only:
        cf_WSE_diff = axs[2, 2].contourf(X, Y, WSE_diff, levels, vmin=min, vmax=max, cmap=plt.cm.jet, extend='both')
    else:
        cf_WSE_diff = axs[2, 2].contourf(X, Y, ma.masked_array(WSE_diff,1-bInDomain), diff_levels, vmin=v_min, vmax=v_max, cmap=plt.cm.jet, extend='both')
...
def plot_zb_inversion_result(zb_inverted_result_filename, config_filename, bPlot_dimless=False):
    ...
    bInDomain = np.load("bInDomain.npz")
    bInDomain = bInDomain['bInDomain']
    ...
    if bPlot_dimless:
        ...
        cf_zb_truth = axs[0, 0].contourf(X, Y, ma.masked_array(scale_back(zb_truth, zb_min, zb_max),1-bInDomain), local_levels,
    ...
    if bPlot_dimless:
        ...
        cf_zb_inverted = axs[0, 1].contourf(X, Y, ma.masked_array(scale_back(zb_inverted_mean, zb_min, zb_max),1-bInDomain), local_levels,
    ...
    if bPlot_dimless:
        ...
        cf_zb_diff = axs[0, 2].contourf(X, Y, ma.masked_array(zb_diff,1-bInDomain), diff_levels, vmin=zb_min, vmax=zb_max, cmap=plt.cm.terrain,
                                        extend='both')  # cm: PRGn
    ...
    for i in range(nSamples):
        ...
        if bPlot_dimless:
            ...
            cf_zb_truth = axs[i + 1, 0].contourf(X, Y, ma.masked_array(scale_back(zb_init_all[:, :, i], zb_min, zb_max),1-bInDomain), local_levels, vmin=zb_min, vmax=zb_max,
        ...
        if bPlot_dimless:
            ...
            cf_zb_inverted = axs[i + 1, 1].contourf(X, Y, ma.masked_array(scale_back(zb_inverted_all[:, :, i], zb_min, zb_max),1-bInDomain),
        ...
        if bPlot_dimless:
            ...
            cf_zb_diff = axs[i + 1, 2].contourf(X, Y, ma.masked_array(zb_diff,1-bInDomain), local_levels, vmin=zb_min, vmax=zb_max, cmap=plt.cm.terrain,
                                                extend='both')  # cm: PRGn
...
def animate_zb_inversion_process(config_filename):
    ...
    zbs = np.load('zb_inverted_result_1000.npz')
    zb_init_all = zbs['zb_init']
    zb_truth = zbs['zb_truth']
    zb_inverted_all = zbs['zb_inverted']
    zb_inverted_all = zbs['zb_inverted']
...
def plot_L_curve(losses_L_curve_file_name):
    ...
    # Annotations
    plt.text(q5+0.1e-1, 2e-1, "5th", size=12, alpha=1)
    plt.text(q50+0.1e-1, 2e-1, "50th percentile of \n surrogate prediction\n error L2 norm", size=12, alpha=1)
    plt.text(q95-1.5, 2e-1, "95th percentile of \n surrogate prediction\n error L2 norm", size=12, alpha=1)
